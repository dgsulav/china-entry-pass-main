...
function captureSignature(
  ...
  if (!wgssSignatureSDK || null == dynCapt) {
  ...
  function onGetInitialHash() {
    ...
    var reason = "Nepal-China Entry / Exit Pass ";
  ...
  function onRenderBitmap(sigObjV, bmpObj, status) {
    if (wgssSignatureSDK.ResponseStatus.OK == status) {
      ...
      //   }
      let signature = bmpObj.image.currentSrc;
      const updated = signatures?.map((sign, i) => {
        if (i === pos) {
          return signature;
        } else {
          return sign;
  ...
  function onGetSigText(sigObjV, text, status) {
    if (wgssSignatureSDK.ResponseStatus.OK == status) {
      ...
      const updated = signaturesText?.map((sign, i) => {
        if (i === pos) {
      ...
      setSignaturesText(updated);
      // setSignaturesText(oldSignaturesText);
...
//   //   }
//   // }
//   let updatedSignatures = signatures.splice(pos, 1, "");
//   let updatedSignaturesText = signaturesText.splice(pos, 1, "");
